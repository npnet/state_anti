...
/* The routine MD5Final terminates the message-digest computation and
   ...
   例程MD5Final终止消息摘要计算，并以mdContext-> digest [0 ... 15]中的所需消息摘要结束。
 */
void modbus_md5_final (uint8_t *hash, hash_md5_ctx_t *mdContext)
{
  ...
  modbus_md5_update (mdContext, PADDING, padLen);
  ...
  copy_buff(hash, mdContext->digest, 16);
}

/* Basic MD5 step. Transforms buf based on in. MD5基本步骤。 根据in转换buf。
 */
static void trans_form (uint32_t *buf, uint32_t *in)
{
