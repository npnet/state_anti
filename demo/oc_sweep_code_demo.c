    ...
    while (1)
    {
        ...
        if (fibo_camera_GetPreviewBuf(&pCamPreviewDataBuffer) == 0)
        {
            ...
            if (count > 20)
                break;
#endif
            ret = fibo_sweep_code(&camctx, pCamPreviewDataBuffer, g_Databuf, aucResult, &ulReslen);
            if (ret)
            {
                if (ulReslen > 0)
                {
            ...
            ((height == 480) && (width == 640)))
    {
        if (!camctx.Openstat)
        {
            ...
            if (!camctx.gCamPowerOnFlag)
            {
                ...
                fibo_thread_create(prvThreadEntry, "sweep code", 1024*8, &camctx, OSI_PRIORITY_NORMAL);
            }
        }
    }
}
