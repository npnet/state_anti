...
static void prvThreadEntry_2(void *param)
{
	...
	int ret = -1;
	UINT8 ip[50];
	char *sbuf = "demo opencpu test tcp ok";
	
    OSI_LOGI(0, "application thread enter, param 0x%x", param);

    fibo_taskSleep(10000);
	ret = fibo_PDPActive(1, NULL, NULL, NULL, 0, 0, ip);
	OSI_PRINTFI("[SOCK][%s-%d] ret = %d,ip=%s", __FUNCTION__, __LINE__, ret,ip);
	fibo_taskSleep(5000);
...
begin_send:
	...
	ret = fibo_sock_send(socketid, (UINT8 *)g_send_test, 4096);
	if(ret < 0){
		OSI_PRINTFI("[%s-%d]send error, errno = %d", __FUNCTION__, __LINE__, errno);
		goto error_exit;
	}
	OSI_PRINTFI("[%s-%d]send buffer OK, buffer = %s, len = %d", 
						__FUNCTION__, __LINE__, sbuf, ret);
    ...
    while(1)
    {
		...
		else if(FD_ISSET(socketid, &readfds))
		{
			...
			char *tmp = rcvbuf + 800;
			OSI_PRINTFI("[%s-%d]recv is OK, rbuf = %s, len = %d\n", __FUNCTION__, __LINE__, rcvbuf, ret);
