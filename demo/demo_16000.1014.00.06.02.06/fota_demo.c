...
void app_download_test()
{
	...
	if(ret < 0){
		...
		OSI_PRINTFE("error code is %d", fibo_ota_errcode());
		goto fexit;
	}
...
void app_update_test()
{
	...
	//char *url = "http://47.97.41.246:8080/ZLF01files/zr_flashV02_1596091410.img";

	//ret = fibo_open_ota(1, "ftp://47.110.234.36:21", "/test1/lobin/hello_flash.img", "httpfota", "test");
	ret = fibo_open_ota(0, url, NULL, NULL, NULL);
	if(ret < 0){
...
void appfota_update_test()
{
	...
	//ret = fibo_open_ota(1, "ftp://47.110.234.36:21", "/test1/lobin/hello_flash.img", "httpfota", "test");
	ret = fibo_appfota_ota(0, url, NULL, NULL, NULL);
	if(ret < 0){
		OSI_PRINTFE("fibo_appfota_ota fail.");
		OSI_PRINTFE("appfota error code = %d", fibo_ota_errcode());
		goto fexit;
	}
	OSI_PRINTFE("fibo_appfota_ota is OK.");
	
	for (int n = 0; n < 500; n++)
    {
        OSI_LOGI(0, "new.bin print here %d", n);
        fibo_taskSleep(1000);
    }
...
void firmware_update_test()
{
	...
	ret = fibo_firmware_ota(0, "ftp://47.110.234.36:21", "/test1/lobin/hello_flash.img", "httpfota", "test");
	if(ret < 0){
...
void appfw_update_test()
{
	...
	//ret = fibo_open_ota(1, "ftp://47.110.234.36:21", "/test1/lobin/hello_flash.img", "httpfota", "test");
	ret = fibo_appfw_ota(0, url, NULL, NULL, NULL);
	if(ret < 0){
		OSI_PRINTFE("fibo_appfw_ota fail.");
		OSI_PRINTFE("appfota error code = %d", fibo_ota_errcode());
		goto fexit;
	}
	OSI_PRINTFE("fibo_appfw_ota is OK.");
