...
static void prvThreadEntry(void *param)
{
    ...
    for(int i = 10;i>0;i--)
    {
		...
		INT32 socket_block = fibo_sock_lwip_fcntl(socketid,F_GETFL,0);
		
	    //default block
		OSI_PRINTFI("[%s-%d] socket_block = %x,O_NONBLOCK=%x\n", __FUNCTION__, __LINE__, socket_block,O_NONBLOCK);
		...
		{
			{
			  ...
			  OSI_PRINTFI("[%s-%d] socket_block_status = %x,O_NONBLOCK=%x\n", __FUNCTION__, __LINE__, fibo_sock_lwip_fcntl( socketid, F_GETFL, 0 ),O_NONBLOCK);
		...
		OSI_PRINTFI("[%s-%d]retcode=%d", __FUNCTION__, __LINE__,retcode);
		
		if(retcode < 0)
		{
			OSI_PRINTFI("[%s-%d],sockets change to non block,TCP socket connect is progressing ,TEST pass", __FUNCTION__, __LINE__);
		}
		else
		{
			OSI_PRINTFI("[%s-%d],sockets change to  block,TCP socket establish success ,TEST pass", __FUNCTION__, __LINE__);
		}
		{
			
			//after a while ,TCP sockets connection success, test send TCP data success
			fibo_taskSleep(5000);
	
	
			memset(g_send_test, '1', 4096); 
			ret = fibo_sock_send(socketid, (UINT8 *)g_send_test, 1020);
			if(ret < 0){
				OSI_PRINTFI("[%s-%d]send error, errno = %d", __FUNCTION__, __LINE__, errno);
			}
			OSI_PRINTFI("[%s-%d]send buffer OK", __FUNCTION__, __LINE__,ret);
	
			fibo_taskSleep(1000);
	
			//after a while ,recv TCP packets success if server send TCP data automatically
			ret = fibo_sock_recv(socketid, (UINT8 *)rcvbuf, 1024);
			if(ret <= 0){
				OSI_PRINTFI("[%s-%d]recv error, errno = %d\n", __FUNCTION__, __LINE__, errno);
			}
			char *tmp = rcvbuf + 1000;
			OSI_PRINTFI("[%s-%d]recv is OK, tmp = %s, len = %d\n", __FUNCTION__, __LINE__, tmp, ret);
...
void *appimg_enter(void *param)
{
	...
	fibo_thread_create(prvThreadEntry, "mythread2", 1024*8*2, NULL, OSI_PRIORITY_NORMAL);
