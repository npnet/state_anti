 ...
 * @File    : data_collector_parameter_table.c
 * @Author  : 
 * @Date    : 2020-08-20
 * @Brief   :
 ******************************************************************************/
...
static u8_t setflag(struct _ParamDescripTable_t *element, void *buf, u16_t *paramLen) {
  *paramLen = Ql_strlen(buf);
  Ql_memcpy(element->a, buf, Ql_strlen(buf));
  return 0;
}
...
static u8_t getflag(struct _ParamDescripTable_t *element, void *buf, u16_t *paramLen) {
  *paramLen = Ql_strlen(element->a);
  Ql_memcpy(buf, element->a, Ql_strlen(element->a));
  return 0;
}
